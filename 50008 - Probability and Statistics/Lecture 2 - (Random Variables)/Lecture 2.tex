...
\begin{document}
    ...
    \section*{Introduction}
        ...
        \subsection*{Cumulative Distributions}
            \termdef{Cumulative Distribution Function ($F_X$)}{
                The cumulative distribution function (cfd) of random variable $X$ is the probability it takes some value less than or equal to some $x$:
    ...
    \section*{Distributions}
        ...
        \subsection*{Discrete Random Variable}
            ...
            \[p_X(x) = P(X = x) = P(E_i) \text{ where } x \in supp(X) \text{ and } x \text{ is the outcome of event } E_i\]
            We can also define using \keyword{cfds}:
            \[F_X(x) = \sum_{x_i \leq x}p_X(x_i) \Leftrightarrow p_X(x_i) = F_X(x_i) - F_X(x_{i-1}) \ \text{ where } i = 2, 3 \dots\]
            Or more simply:
            \[p_X(x) = P_X(X = x_i) = P(X \leq x_i) - P(X \leq x_{i-1}) = F_X(x_i) - F_X(x_{i-1})\]
        ...
        \subsection*{Variance}
            \termdef{Moment}{
                ...
                \compitem{
                    ...
                    \bullpara{Central Moment}{The variance ($E(X - E(X))$)}
            ...
            \termdef{Variance}{
                ...
                \[Var(X) = Var_X(X) = \sigma_X^2 = E(X - E(X)) = E(X^2) - (E(X))^2\]
                Note that:
