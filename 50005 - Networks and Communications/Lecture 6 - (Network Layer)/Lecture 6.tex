...
\termdef{Shortest Path Routing (SPR)}{
	Use \keyword{Djikstra's Algorithm} to determine the shorest routes from each router, to every destination. The forward packets accordingly.
}
...
However there is a \keyword{count-to-infinity} problem. When a node goes down, and routers continually update their costs based off seachothers, resulting in the cost incrementing constantly.
\[\begin{split}
		A \to C &= B \to C + 1 \\
		B \to C &= A \to C + 1 \\
	\end{split}\]
...
\termdef{Link State Routing}{
	...
	\compenum {
		...
		\item Recieve \keyword{LSA} packets from every other reouter on the network.
...
\termdef{Open Shortest Path First (OSPF)}{
	...
	\compitem{
		...
		\item Offsers some scurity features (though some have been compromised).
	}
